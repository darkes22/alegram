...
<?php 
	...
	$sql="SELECT fotos.id, fotos.id_usuario, fotos.titulo, fotos.descripcion, fotos.link, usuarios.nombre, usuarios.alias FROM fotos, usuarios where fotos.id_usuario=usuarios.id and usuarios.status=1 order by id desc limit $ini,12";
	$q=mysqli_query($conex,$sql);
	while ($ver=mysqli_fetch_array($q)) { ?>
						<li id="zoom">
							<a href="#">
								<img src="<?php echo $ver[4]; ?>"/>
								<div class="desc">
									<h3> <?php echo "By: ".$ver[5]; ?> <br> <?php echo $ver[6]; ?></h3>
								</div>
								</a>
						</li>
						<?php } ?>
